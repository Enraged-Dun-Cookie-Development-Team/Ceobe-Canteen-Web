<template>
    <canvas class="spine" ref="canvasSpine" :style="{zIndex: 9999}"></canvas>
    <div 
...
const redirectSpine = () => {
  const w = canvas.clientWidth
  const h = canvas.clientHeight
  const position = props.spinePosition
  spineboy.skeleton.x = position?.left ? -w*scale+position?.left : w*scale-position?.right
  spineboy.skeleton.y = position?.top ? h*scale-position?.top : -h*scale+position?.bottom
}
...
// 拖拽小人实现移动
const spinedrag = (e:any) => {
  document.onselectstart = function() { return false }
  let initX = e.clientX
  let initY = e.clientY 
  spinePosition.value!.onmousemove = (e: any) => {
    document.onselectstart = function() { return false }
    ...
    if (state.left <= 0 || 
      state.top <= 0 || 
      state.left >= canvas.clientWidth - state.bonesPositionXMax || 
      state.top >= canvas.clientHeight - state.bonesPositionYMax) {
      spinePosition.value!.onmousemove = null
      spinePosition.value!.onmouseup = null
    }
  }
  spinePosition.value!.onmouseup = () => {
    spinePosition.value!.onmousemove = null
    spinePosition.value!.onmouseup = null
  }
}
